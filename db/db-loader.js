	...
	init((err)=>{
		if(!err){
			moduleLoader(path.join(__dirname, 'master.collections'),'.collection.js','master db',(err,holder)=>{
				if(!err){
					global.db=holder
					moduleLoader(path.join(__dirname, 'client.collections'),'.collection.js','master db',(err,holder)=>{
						if(!err){
							global.clientDbModels=holder
...
exports.connectDatabase=function(_id,userDb,userDbHost,dbName,cb){
    ...
    usrConn.on('connected', function () {  
	    ...
	    repoDb[_id]={}
	    Object.keys(clientDbModels).forEach((e)=>{
	    	repoDb[_id][e]=clientDbModels[e](usrConn)
	    })
